...
This version contains breaking changes — APIs, conventions, and file structure may all differ from your training data. Read the relevant guide in `node_modules/next/dist/docs/` before writing any code. Heed deprecation notices.
...
8. On approval, re-read the approved prompt file in the prompts folder and implement it strictly. Implement only after user approval.
...
- `node_modules/next/dist/docs/`: Next.js, routing, server/client boundaries, API routes, and UI patterns
- `clerk`: authentication and protected routes
- `supabase`: schema, migrations, queries, service role usage, dedupe, logs, and pgvector
- `oxylabs-web-scraper`: Oxylabs Web Scraper API, Scheduler, scheduled jobs, and scraping behavior
- `ai-sdk`: Vercel AI SDK, OpenAI provider usage, model calls, and AI analysis output handling

Do not invent new skills.

For Cheerio, Zod, Tailwind, and shadcn/ui, use existing project patterns, package documentation, and `node_modules/next/dist/docs/`.
...
- Parsing: article link extraction, cleanup, and article validation
...
- left percentage, center percentage, and right percentage (each 0–100; must sum to 100)
- confidence (0 to 1)
- framing notes
- loaded terms
- disclaimer
...
One exception — the Vercel Cron route uses `GET` because Vercel Cron sends GET requests:
...
Always read these IDs from the raw HTTP response text before any `JSON.parse` call — use string extraction or a regular expression on the raw text to capture the exact digit sequence. Never convert a parsed JavaScript number back to a string; precision is already lost at parse time.
...
- `POST /api/oxylabs/schedules` — tells Oxylabs what to scrape hourly. Run this once for the current active source set and again when the active source set changes.
- Vercel Cron config — tells Vercel to call `/api/cron/pipeline` at :15 past every hour. Configure this once via `vercel.json`.
...
5. Step two: immediately run AI analysis on all newly inserted articles that are still pending analysis or pending embedding.
6. If step one fails, step two must still run — there may be pre-existing unanalyzed articles or articles requiring embedding backfill.
...
Protect the cron route using the `CRON_SECRET` environment variable. Configure the secret in the Vercel project's Environment Variables and use it to authenticate incoming cron requests. Reject requests with a missing or wrong value with `401`.

In local development, skip the secret check so the route can be tested manually.

Do not use `BIASLY_ADMIN_SECRET` to protect the cron route. Do not commit `CRON_SECRET` to source control or expose it to client code.
...
AI analysis must process valid articles missing required analysis or embedding data, detected by the **pending-analysis** and **pending-embedding** checks in the Required Behavior list below — based on the actual state of `article_analyses`, not `analyzed_at` alone.
...
**Terminology:** “political framing” is the user-facing concept. Existing database fields retain the `bias_*` names.

Political framing must be shown as **AI-estimated**, not objective truth.

Framing output rules:

- `leftPercentage`, `centerPercentage`, and `rightPercentage` must be integers from 0 to 100.
- The three percentages must add up to exactly 100.
- `politicalFramingLabel` must be one of: `left`, `center`, `right`, `mixed`, or `unclear`.
- The label should normally match the strongest percentage.
- Treat the top two percentages as “close” when they differ by less than 10 percentage points; when they are close, or confidence is low, the label may be `mixed` or `unclear` based on the evidence.
...
- Validate AI output with Zod or an equivalent validator before saving.
- If output is invalid, retry once or mark the article as failed without saving bad analysis.

## Required behavior

1. **Pending-analysis check** — detect pending articles by `LEFT JOIN`ing `articles` to `article_analyses`. An article has pending analysis when no `article_analyses` row exists for it. Never rely on `analyzed_at IS NULL` alone.
2. **Pending-embedding check** — detect articles that have a valid `article_analyses` row but `embedding IS NULL`. These articles require embedding generation without re-running the full analysis unless the analysis itself is invalid or missing required fields.
3. Process work in configurable batches.
4. Continue until no pending analysis or pending embedding work remains for full analysis runs.
5. Validate AI output before saving.
6. Save analysis only for valid articles.
7. Generate and save the embedding when required.
8. Mark `analyzed_at` only after all required analysis data and the embedding have been saved successfully.
9. Log analyzed, embedded, skipped, and failed counts per batch and in the final summary.
10. Log concise console progress during the run.
11. Log a final summary object when complete.
...
Update the `/api/analyze` route to also call OpenAI `text-embedding-3-small` for each article alongside the existing analysis call and save the result to `article_analyses.embedding`.

For an article with no analysis row, generate and save both the analysis and embedding before setting `analyzed_at`.

For an article whose analysis row exists but whose embedding is `NULL`, generate only the missing embedding. Do not re-run the full analysis unless the analysis is otherwise invalid or incomplete.

Update `analyzed_at` only after the required analysis data and embedding have been saved successfully.
...
`CRON_SECRET` is configured as a Vercel Environment Variable and is used to authenticate `GET /api/cron/pipeline`. It must not be exposed to browser code or committed to source control.
...
| `CRON_SECRET`                                                                 | Protects `GET /api/cron/pipeline`; configured in Vercel Environment Variables (section 18) | server only     |
